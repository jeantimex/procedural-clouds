...
// ------------------------------------------------------------
// FAST VORONOI (2x2x2x2 neighborhood)
// ------------------------------------------------------------

fn voronoi_f1_fast(params: VoronoiParams, coord: vec4f) -> f32 {
    let cellPosition_f = floor(coord);
    let localPosition = coord - cellPosition_f;
    let cellPosition = vec4i(cellPosition_f);
        ...
        for (var k = 0; k <= 1; k++) {
            for (var j = 0; j <= 1; j++) {
                for (var i = 0; i <= 1; i++) {
                    ...
                    let pointPosition = vec4f(cell - cellPosition) + hash_int4_to_vec4(cell) * params.randomness;
                    let distanceToPoint = voronoi_distance(pointPosition, localPosition, params);
                    if (distanceToPoint < minDistance) {
                        minDistance = distanceToPoint;
                    }
...
fn fractal_voronoi_x_fx_fast(params: VoronoiParams, coord: vec4f) -> f32 {
    var amplitude = 1.0;
    var max_amplitude = 0.0;
    var scale = 1.0;
    ...
    for (var i = 0; i <= max_i; i++) {
        let octave = voronoi_f1_fast(params, coord * scale);
        if (zero_input) {
            max_amplitude = 1.0;
            distance_sum = octave;
            break;
            ...
            if (remainder != 0.0) {
                max_amplitude = mix(max_amplitude, max_amplitude + amplitude, remainder);
                distance_sum = mix(distance_sum, distance_sum + octave * amplitude, remainder);
            }
        }
    }

    if (params.normalize) {
        distance_sum /= max_amplitude * params.max_distance;
    }

    return distance_sum;
}

fn node_tex_voronoi_f1_4d_distance_fast(
    coord: vec3f,
    w: f32,
    scale: f32,
    detail: f32,
    ...
    let coord_scaled = coord * scale;
    params.max_distance = voronoi_distance(vec4f(0.0), vec4f(0.5 + 0.5 * params.randomness), params);
    return fractal_voronoi_x_fx_fast(params, vec4f(coord_scaled, w_scaled));
}
